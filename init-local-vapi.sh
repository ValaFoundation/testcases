...
START_MARKER="# >>> vala-testcases local setup >>>"
END_MARKER="# <<< vala-testcases local setup <<<"
...
echo -e "${BLUE}==> Installing vala-testcases into local project folders...${NC}"
...
try_download_release_zip() {
  local out_zip="$1"
  local tag_zip_url="https://github.com/${GITHUB_REPO}/releases/download/${REPO_REF}/vala-testcases-${REPO_REF}-linux.zip"
  ...
  latest_zip_url="$(curl -fsSL "${latest_api_url}" | grep -o 'https://[^\"]*vala-testcases-[^\"]*-linux\.zip' | head -n 1 || true)"
...
SOURCE_VAPI="${SOURCE_DIR}/vapi/vala_testcases.vapi"
SOURCE_HEADER="${SOURCE_DIR}/vala_testcases.h"

if [ ! -f "${SOURCE_VAPI}" ] || [ ! -f "${SOURCE_HEADER}" ] || ! compgen -G "${SOURCE_DIR}/libvala_testcases.so*" > /dev/null; then
...
cp -a "${SOURCE_DIR}"/libvala_testcases.so* "${LIB_DIR}/"

if ! grep -Fq "${START_MARKER}" "${MESON_FILE}"; then
    echo -e "${BLUE}==> Appending helper block to meson.build...${NC}"
    cat <<'EOF' >> "${MESON_FILE}"

# >>> vala-testcases local setup >>>
vala_testcases_local_deps = [
  dependency('glib-2.0'),
  dependency('gee-0.8'),
]

vala_testcases_local_vala_args = [
  '--vapidir=' + meson.project_source_root() / 'vapi',
]

vala_testcases_local_c_args = [
  '-I' + meson.project_source_root() / 'include',
]

vala_testcases_local_link_args = [
...
echo -e "  dependencies: vala_testcases_local_deps"
echo -e "  vala_args: vala_testcases_local_vala_args"
echo -e "  c_args: vala_testcases_local_c_args"
echo -e "  link_args: vala_testcases_local_link_args"
echo -e ""
